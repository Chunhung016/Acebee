...
export const PublicDashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-6rem)] pb-16">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {pinnedAnnouncements.map((ann) => (
                    ...
                    >
                      {ann.imageUrl && (
                        <div className="relative overflow-hidden rounded-t-xl bg-slate-900/5">
                          <AnnouncementImage
                            variant="pinned"
                            src={ann.imageUrl}
                            alt={ann.title}
                          ...
                          <div className="absolute top-3 right-3 z-20">
                            <span
                              className={`text-[11px] font-bold px-2.5 py-1 rounded-md border shadow-xs bg-white/95 backdrop-blur-xs ${getCategoryBadgeClass(
                                ann.category
                              )}`}
                            >
                              {ann.category}
                    ...
                    {ann.imageUrl && (
                      <div className="relative overflow-hidden rounded-t-xl bg-slate-900/5">
                        ...
                        <div className="absolute top-3 left-3 z-20">
                          <span
                            className={`text-xs font-bold px-2.5 py-1 rounded-md border shadow-xs bg-white/95 backdrop-blur-xs ${getCategoryBadgeClass(
                              ann.category
                            )}`}
                          >
                            {ann.category}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs animate-in fade-in duration-200">
          ...
          >
              ...
              <div className="space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {activeAnnouncement.pinned && (
                    <span className="px-2 py-0.5 rounded-md bg-blue-500/20 text-blue-300 border border-blue-400/30 text-xs font-semibold flex items-center gap-1">
                      <Pin className="w-3 h-3 fill-blue-300" />
                      Pinned Notice
                    </span>
                  )}
                  {activeAnnouncement.badge && (
                    <span className="px-2 py-0.5 rounded-md bg-amber-500/20 text-amber-300 border border-amber-400/30 text-xs font-semibold">
                      {activeAnnouncement.badge}
                    </span>
                  )}
                </div>
                <h2 className="text-xl sm:text-2xl font-bold font-['Plus_Jakarta_Sans',sans-serif] leading-snug">
                  {activeAnnouncement.title}
                </h2>
                <div className="flex items-center gap-2 text-xs text-slate-400">
                  <Calendar className="w-3.5 h-3.5 text-slate-400" />
                  <span>Published on {formatDate(activeAnnouncement.createdAt)}</span>
                </div>
              </div>
              <button
                onClick={() => setActiveAnnouncement(null)}
                className="text-slate-400 hover:text-white p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors shrink-0"
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              {/* High-Resolution Uncropped Image Presentation */}
              {activeAnnouncement.imageUrl && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs text-slate-500">
                    <span className="font-semibold text-slate-700 flex items-center gap-1.5">
                      <ImageIcon className="w-3.5 h-3.5 text-blue-600" />
                      Official Graphic / Poster
                    </span>
                    <span className="text-[11px] text-slate-400">Click to view full screen</span>
                  </div>
                  <AnnouncementImage
                    variant="modal"
                    src={activeAnnouncement.imageUrl}
                    alt={activeAnnouncement.title}
                    onEnlarge={() => setLightboxImage({ src: activeAnnouncement.imageUrl!, alt: activeAnnouncement.title })}
                  />
                </div>
              )}
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm sm:text-base text-slate-800 leading-relaxed whitespace-pre-line bg-slate-50 p-4 rounded-xl border border-slate-100">
      ...
      {/* Full-Screen High-Res Image Lightbox */}
      {lightboxImage && (
        <ImageLightboxModal
